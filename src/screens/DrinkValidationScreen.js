...
const DrinkValidationScreen = ({ navigation }) => {
  ...
  // Filter drink items
  const drinkItems = qrData?.food?.items?.filter(item => 
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerContent}>
            <Text style={styles.headerTitle}>Validación de Bebestibles</Text>
            <Text style={styles.headerSubtitle}>Sin bebestibles registrados</Text>
          </View>
        </View>
        
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyIcon}>🥤</Text>
          <Text style={styles.emptyTitle}>Sin bebestibles</Text>
          <Text style={styles.emptyDescription}>
            Esta compra no incluye bebestibles para canjear.
          </Text>
        </View>
      </View>
    );
  ...
  const totalItems = drinkItems.reduce((sum, item) => sum + item.cantidad, 0);
  const redeemedItems = drinkItems.reduce((sum, item) => {
    const remaining = validationStates.food[item.id] || 0;
    return sum + (item.cantidad - remaining);
  }, 0);

  // Calculate subtotal for drinks only
  const drinkSubtotal = drinkItems.reduce((sum, item) => sum + item.subtotal, 0);
      ...
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          ...
          <View style={styles.summaryCard}>
            <Text style={styles.summaryTitle}>📊 Resumen de Bebestibles</Text>
          ...
          <View style={styles.itemsContainer}>
            <Text style={styles.sectionTitle}>🥤 Lista de Bebestibles</Text>
...
export default DrinkValidationScreen;
